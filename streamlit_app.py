...
def calculate_attendance_percentages(start_date_str):
    ...
    if total_days == 0:
        return pd.DataFrame(columns=['Roll', 'Percentage'])

    attendance_percentage = df_filtered.groupby('Roll').size() / total_days * 100
    return attendance_percentage.reset_index().rename(columns={0: 'Percentage'})
...
def main():
    ...
    # Register New Student
        ...
        if st.button("Register"):
            if newusername and newuserid and newuserbranch:
                user_folder = f'static/faces/{newusername}_{newuserid}_{newuserbranch}'
    ...
    # Mark Attendance
        ...
        while True:
            ...
            if len(faces) > 0:
                ...
                identified_person = model.predict(face)
                identified_person = identified_person[0]
                add_attendance(identified_person)

                cv2.rectangle(frame, (x, y), (x+w, y+h), (0, 255, 0), 2)
                stframe.image(frame, channels="BGR", caption=f"Attendance Marked for {identified_person}")
                break
    ...
    # View Attendance + Percentage with Download Option
        ...
        start_date = st.date_input("Select Start Date", date(2025, 1, 1))
